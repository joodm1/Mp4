...
export async function GET(request: Request): Promise<NextResponse> {
    ...
    // If no 'city' parameter is provided, return a 408 Bad Request error response
    if (!city) {
        return NextResponse.json({ error: "No [city] provided" }, { status: 408 });
    }
    ...
    // If the API request fails (status code other than 200), return a 560 Internal Server Error response
    if (res.status !== 200) {
        return NextResponse.json({ error: "Failed to fetch data" }, { status: 560 });
    }
